...
def send_object_list(object_list, dst, group=None, device=None):
    ...
    # sent to this device.
    current_device = device or torch.distributed.distributed_c10d._get_pg_default_device(
        group)
    # Serialize object_list elements to tensors on src rank.
    tensor_list, size_list = zip(*[
        torch.distributed.distributed_c10d._object_to_tensor(
            obj, current_device) for obj in object_list
    ])
...
def recv_object_list(object_list, src=None, group=None, device=None):
    ...
    rank_sizes = torch.distributed.recv(object_sizes_tensor,
                                        src=src,
                                        group=group)
